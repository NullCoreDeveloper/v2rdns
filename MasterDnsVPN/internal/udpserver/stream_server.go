...
func (s *Stream_server) enqueueInboundData(packetType uint8, sequenceNum uint16, fragmentID uint8, totalFragments uint8, payload []byte) bool {
	if s == nil || s.ARQ == nil {
		return false
	}

	s.ARQ.QueueRXPacket(packetType, sequenceNum, fragmentID, totalFragments, payload)
	return true
}
