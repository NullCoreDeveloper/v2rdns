...
const (
	...
	SessionAcceptPolicyPayloadSize = 14
...
//	bytes 8-9   : max upload MTU (uint16)
//	bytes 10-11 : max download MTU
//	byte 12     : max RX/TX workers
//	byte 13     : min ping aggressive interval (scaled 0..255 => 0.05..1.00s)
//	byte 14     : max packets per batch
//	bytes 15-16 : max ARQ window size
//	byte 17     : max ARQ data NACK max gap
//	bytes 18-19 : min compression min size
//	byte 20     : min ARQ initial RTO (scaled 0..255 => 0.05..1.00s)
...
//
//	byte 7  : lower nibble = max packet duplication, upper nibble = max setup duplication
//	bytes 8-9 : max upload MTU (uint16, big endian)
//	bytes 10-11 : max download MTU (uint16, big endian)
//	byte 12 : max RX/TX workers (uint8)
//	byte 13 : min ping aggressive interval, scaled 0..255 => 0.05..1.00 seconds
//	byte 14 : max packets per batch (uint8)
//	bytes 15-16 : max ARQ window size (uint16, big endian)
//	byte 17 : max ARQ data NACK max gap (uint8)
//	bytes 18-19 : min compression min-size (uint16, big endian)
//	byte 20 : min ARQ initial RTO, scaled 0..255 => 0.05..1.00 seconds
...
func EncodeSessionAcceptClientPolicy(policy SessionAcceptClientPolicy) [SessionAcceptPolicyPayloadSize]byte {
	...
	binary.BigEndian.PutUint16(payload[1:3], uint16(clampInt(policy.MaxUploadMTU, 0, 0xFFFF)))
	binary.BigEndian.PutUint16(payload[3:5], uint16(clampInt(policy.MaxDownloadMTU, 0, 0xFFFF)))
	payload[5] = byte(clampInt(policy.MaxRxTxWorkers, 0, 0xFF))
	payload[6] = EncodeSessionScaledByte(policy.MinPingAggressiveInterval)
	payload[7] = byte(clampInt(policy.MaxPacketsPerBatch, 0, 0xFF))
	binary.BigEndian.PutUint16(payload[8:10], uint16(clampInt(policy.MaxARQWindowSize, 0, 0xFFFF)))
	payload[10] = byte(clampInt(policy.MaxARQDataNackMaxGap, 0, 0xFF))
	binary.BigEndian.PutUint16(payload[11:13], uint16(clampInt(policy.MinCompressionMinSize, 0, 0xFFFF)))
	payload[13] = EncodeSessionScaledByte(policy.MinARQInitialRTOSeconds)
...
func DecodeSessionAcceptClientPolicy(payload []byte) (SessionAcceptClientPolicy, error) {
	...
	return SessionAcceptClientPolicy{
		...
		MaxUploadMTU:              int(binary.BigEndian.Uint16(payload[1:3])),
		MaxDownloadMTU:            int(binary.BigEndian.Uint16(payload[3:5])),
		MaxRxTxWorkers:            int(payload[5]),
		MinPingAggressiveInterval: DecodeSessionScaledByte(payload[6]),
		MaxPacketsPerBatch:        int(payload[7]),
		MaxARQWindowSize:          int(binary.BigEndian.Uint16(payload[8:10])),
		MaxARQDataNackMaxGap:      int(payload[10]),
		MinCompressionMinSize:     int(binary.BigEndian.Uint16(payload[11:13])),
		MinARQInitialRTOSeconds:   DecodeSessionScaledByte(payload[13]),
	}, nil
}
