...
func (c *Client) HandleDNSQueryRes(packet VpnProto.Packet) error {
	...
	c.dnsCallbacksMu.Lock()
	entry, exists := c.dnsCallbackEntries[packet.SequenceNum]
	if exists {
		delete(c.dnsCallbackEntries, packet.SequenceNum)
		delete(c.dnsCallbacks, packet.SequenceNum)
	}
	c.dnsCallbacksMu.Unlock()

	if exists && entry.cb != nil {
		go entry.cb(assembled)
	}
...
func (c *Client) ProcessDNSQuery(query []byte, addr net.Addr, respond func([]byte)) bool {
	...
	sn := uint16(c.dnsSeqCounter.Add(1) & 0xFFFF)

	if respond != nil {
		cb := func(resp []byte) {
			patched := dnsCache.PatchResponseForQuery(resp, query)
			respond(patched)
		}
		c.dnsCallbacksMu.Lock()
		// Если старый callback с этим же SN не отработал — вызываем его, чтобы не утекал
		if old, exists := c.dnsCallbackEntries[sn]; exists && old.cb != nil {
			go old.cb(nil) // nil = timeout/eviction
		}
		c.dnsCallbackEntries[sn] = dnsCallbackEntry{cb: cb, createdAt: time.Now()}
		c.dnsCallbacks[sn] = cb
		c.dnsCallbacksMu.Unlock()
	}

	c.dispatchDNSQueryToTunnel(query, sn)
	return false
}

// cleanStaleDNSCallbacks удаляет callbacks, которые висят дольше maxAge.
// Вызывается периодически, чтобы не было утечки памяти.
func (c *Client) cleanStaleDNSCallbacks(maxAge time.Duration) {
	if c == nil {
		return
	}
	now := time.Now()
	c.dnsCallbacksMu.Lock()
	for sn, entry := range c.dnsCallbackEntries {
		if now.Sub(entry.createdAt) > maxAge {
			delete(c.dnsCallbackEntries, sn)
			delete(c.dnsCallbacks, sn)
		}
	}
	c.dnsCallbacksMu.Unlock()
}

func (c *Client) dispatchDNSQueryToTunnel(query []byte, sn uint16) {
	if !c.SessionReady() {
		return
	}
	...
	fragments := fragmentPayload(query, c.safeUploadMTU)
