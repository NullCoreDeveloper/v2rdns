...
func (c *Client) buildPlannedOutboundFrames(
	...
	if err != nil {
		c.log.Errorf("buildEncodedAutoWithCompressionTrace failed for type %s (0x%X): %v", Enums.PacketTypeName(task.opts.PacketType), task.opts.PacketType, err)
		return nil, err
	}
...
func (c *Client) handleInboundPacket(data []byte, addr *net.UDPAddr, localAddr string) {
	...
	if err != nil {
		...
		if c.log != nil {
			c.log.Infof("🚨 <red>Failed to parse VPN packet from DNS response: %v from %v</red>", err, addr)
		}
		return
	}
	...
	if c.log != nil && vpnPacket.PacketType != Enums.PACKET_PONG {
		if vpnPacket.PacketType == Enums.PACKET_STREAM_DATA_ACK {
			c.log.Infof("Client received ACK | Stream: %d | Seq: %d", vpnPacket.StreamID, vpnPacket.SequenceNum)
		} else {
			c.log.Infof("Client received inbound VPN packet | Packet: %s | Stream: %d | Seq: %d | Payload: %d | Frag: %d/%d",
