...
func TestParseResolversFromJson(t *testing.T) {
	rawJSON := `{
  "LISTEN_PORT": 10808,
  "RESOLVERS": [
    {
  ...
  "ENCRYPTION_KEY": "test-secret"
	...
	if cfg.Resolvers[0].IP != "127.0.0.1" || cfg.Resolvers[0].Port != 53 {
		t.Fatalf("unexpected resolver parsed: IP=%s Port=%d", cfg.Resolvers[0].IP, cfg.Resolvers[0].Port)
	}
}
