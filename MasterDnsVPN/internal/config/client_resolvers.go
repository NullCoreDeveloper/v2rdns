...
func ParseResolverAddress(text string) (ResolverAddress, error) {
	target, port, err := parseResolverEntry(text)
	if err != nil {
		return ResolverAddress{}, err
	}
	if target.isPrefix {
		return ResolverAddress{}, fmt.Errorf("subnet resolver prefixes not supported in JSON")
	}
...
func (r *ResolverAddress) UnmarshalJSON(data []byte) error {
	var str string
	if err := json.Unmarshal(data, &str); err == nil {
		parsed, err := ParseResolverAddress(str)
		if err != nil {
			return err
		}
		*r = parsed
	...
	if addrVal, ok := m["ADDRESS"]; ok {
		if addrStr, ok := addrVal.(string); ok {
			parsed, err := ParseResolverAddress(addrStr)
			if err != nil {
				return err
			}
			*r = parsed
	...
	if ipVal, ok := m["IP"]; ok {
		ip, _ = ipVal.(string)
	} else if ipVal, ok := m["ip"]; ok {
		ip, _ = ipVal.(string)
	}

	var port float64
	if portVal, ok := m["Port"]; ok {
		port, _ = portVal.(float64)
	} else if portVal, ok := m["port"]; ok {
		port, _ = portVal.(float64)
	}

	if ip == "" {
		return fmt.Errorf("missing IP address in resolver")
	}
